# ![[tktk Module Name] - tktk Microlesson Name](./assets/hero.png)

**Learning objective:** By the end of this lesson, students will be able to tktk

tktk Notes: Clone base auth repo at <https://git.generalassemb.ly/modular-curriculum-all-courses/men-stack-session-auth-template> [the name of this repo is still being finalized, don't link this in course materials without reaching out to confirm a final location for it], set up .env, run `npm i`.

tktk: Also, this App needs a name! If inspiration strikes while you're building this out or you have a fun way to theme it 100% lean into it!

tktk Code: The final code for this lecture can be found at: <https://git.generalassemb.ly/modular-curriculum-all-courses/men-stack-applications-tracker>
